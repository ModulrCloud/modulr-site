...
function RelatedCard({ post, visualIdx }: { post: ResearchPost; visualIdx: number }) {
  ...
  return (
    <Link href={`/eleven/research/${post.slug}`} style={{ textDecoration: "none", color: "inherit", display: "block" }}>
      <div style={{
        position: "relative",
        height: 220,
        overflow: "hidden",
  ...
  return (
    <header style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(255,255,255,0.82)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: sectionBorder }}>
      <div style={{ maxWidth: T.maxWFull, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
          ...
          <nav className="el-desktop-only" style={{ gap: 4 }}>
            {navLinks.map((item) => (
              <Link key={item.label} href={item.href} style={{ padding: "6px 12px", fontSize: 14, color: item.href === "/eleven/research" ? T.text : T.muted, fontWeight: item.href === "/eleven/research" ? 600 : 400, textDecoration: "none", borderRadius: 8 }}>
                {item.label}
              </Link>
            ))}
          </nav>
      ...
      <div className={`el-mobile-menu${isMobileNavOpen ? " open" : ""}`}>
        {navLinks.map((item) => (
          <Link key={item.label} href={item.href} onClick={() => setIsMobileNavOpen(false)} style={{ padding: "10px 0", fontSize: 15, color: item.href === "/eleven/research" ? T.text : T.muted, fontWeight: item.href === "/eleven/research" ? 600 : 400, textDecoration: "none" }}>
            {item.label}
          </Link>
        ))}
      </div>
    ...
    return (
      <div style={{ background: T.bg, color: T.text, minHeight: "100vh", display: "flex", flexDirection: "column", fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16, padding: 40 }}>
          ...
          <Link href="/eleven/research" style={{ marginTop: 12, padding: "10px 24px", borderRadius: T.radiusPill, background: T.accent, color: T.accentFg, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>
            ← Back to Research
          </Link>
        </div>
      </div>
    ...
    <div style={{ background: T.bg, color: T.text, fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <article style={{ maxWidth: T.maxW, margin: "0 auto", padding: "60px 24px 80px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Link href="/eleven/research" style={{
            display: "inline-flex",
            alignItems: "center",
            gap: 8,
            fontSize: 14,
