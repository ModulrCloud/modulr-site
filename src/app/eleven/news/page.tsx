    ...
    <div style={{ background: T.bg, color: T.text, fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <header style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(255,255,255,0.82)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: sectionBorder }}>
        <div style={{ maxWidth: T.maxW, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
            ...
            <nav className="el-desktop-only" style={{ gap: 4 }}>
              {navLinks.map((item) => (
                <Link key={item.label} href={item.href} style={{ padding: "6px 12px", fontSize: 14, color: item.href === "/eleven/news" ? T.text : T.muted, fontWeight: item.href === "/eleven/news" ? 600 : 400, textDecoration: "none", borderRadius: 8 }}>
                  {item.label}
                </Link>
              ))}
            </nav>
        ...
        <div className={`el-mobile-menu${isMobileNavOpen ? " open" : ""}`}>
          {navLinks.map((item) => (
            <Link key={item.label} href={item.href} onClick={() => setIsMobileNavOpen(false)} style={{ padding: "10px 0", fontSize: 15, color: item.href === "/eleven/news" ? T.text : T.muted, fontWeight: item.href === "/eleven/news" ? 600 : 400, textDecoration: "none" }}>
              {item.label}
            </Link>
          ))}
        </div>
      ...
      {stories.length > 0 && (
        <section style={{ padding: "0 24px 60px", maxWidth: T.maxW, margin: "0 auto" }}>
          <Link href={stories[0].href} target="_blank" rel="noreferrer" style={{ textDecoration: "none", color: "inherit" }}>
            <div className="el-g-split-wide" style={{ borderRadius: T.radiusXl, overflow: "hidden", cursor: "pointer", transition: "box-shadow 0.2s" }}>
              <div style={{ background: cardGradients[0], minHeight: 380, position: "relative", overflow: "hidden" }}>
                ...
                <img src={stories[0].image} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", filter: "blur(30px) saturate(1.6)", opacity: 0.5, transform: "scale(1.2)" }} />
              ...
              <div style={{ padding: "40px 32px", background: T.surface, display: "flex", flexDirection: "column", justifyContent: "center" }}>
                <div style={{ fontSize: 12, color: T.muted2, marginBottom: 12 }}>{stories[0].meta}</div>
                <h2 style={{ fontSize: 28, fontWeight: 600, letterSpacing: "-0.02em", lineHeight: 1.2, marginBottom: 12 }}>{stories[0].title}</h2>
      ...
      <section style={{ padding: "0 24px 80px", maxWidth: T.maxW, margin: "0 auto" }}>
        <h2 style={{ fontSize: 24, fontWeight: 500, marginBottom: 32 }}>All posts</h2>
        {loading ? (
          <p style={{ fontSize: 14, color: T.muted }}>Loading news...</p>
        ) : stories.length <= 1 ? (
          <p style={{ fontSize: 14, color: T.muted }}>No stories available.</p>
        ) : (
